...
# Train XGBClassifier and return the trained model
def train_xgb_model(X_train_scaled, Y_train_encoded):
...
def main():
    ...
    X = preprocess_data(X)
    print(f"Preprocessed Data: {X.shape}")  # Debug statement

    X_scaled = scale_features(X)
    print(f"Scaled Data: {X_scaled.shape}")  # Debug statement

    label_encoder = LabelEncoder()
    Y_encoded = label_encoder.fit_transform(Y)
    print(f"Encoded Labels: {np.unique(Y_encoded)}")  # Debug statement

    # Split data into training and testing sets
    X_train, X_test, Y_train, Y_test = train_test_split(X_scaled, Y_encoded, test_size=0.2, random_state=42)
    print(f"Training set size: {X_train.shape}, Testing set size: {X_test.shape}")  # Debug statement
    ...
    for threshold in thresholds:
        predictions = (xgb_model.predict_proba(X_test)[:, 1] > threshold).astype(int)
